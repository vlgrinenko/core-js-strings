...
function orderAlphabetically(str) {
  ...
  for (let i = 0; i < arrayChars.length - 1; i += 1) {
    for (let j = 0; j < arrayChars.length - 1; j += 1) {
      if (arrayChars[j] > arrayChars[j + 1]) {
        const max = arrayChars[j];
        arrayChars[j] = arrayChars[j + 1];
        arrayChars[j + 1] = max;
      }
    }
  }
  ...
  let sortFinish = '';
  for (let index = 0; index < arrayChars.length; index += 1) {
    sortFinish += arrayChars[index];
  }
...
function countVowels(str) {
  ...
  const arrayChars = loverStr.split('');
  const arrayVowels = ['a', 'e', 'i', 'o', 'u', 'y'];
  let count = 0;

  for (let i = 0; i < arrayChars.length; i += 1) {
    for (let j = 0; j < arrayVowels.length; j += 1) {
      if (arrayChars[i] === arrayVowels[j]) {
        count += 1;
      }
    }
  }
...
function isPalindrome(str) {
  let reverseStr = '';
  const strReplace = str.replace(/[^\w]/g, '');

  for (let index = strReplace.length - 1; index >= 0; index -= 1) {
    reverseStr += strReplace[index];
  }

  return reverseStr.toLowerCase() === strReplace.toLowerCase();
}
  ...
  return sentence
    ...
    .reduce(
      (longWord, currentWord) =>
        currentWord.length > longWord.length ? currentWord : longWord,
      ''
    );
}
...
function extractNameFromTemplate(value) {
  return value.split(/[,!]/).map((part) => part.trim())[1];
}
...
function encodeToRot13(str) {
  ...
  return str.replace(/[a-zA-Z]/g, (char) => final[initial.indexOf(char)]);
}
...
function getCardId(value) {
  ...
  const indexRaiting = raitings.indexOf(value.slice(0, -1));
  const indexSuit = suits.indexOf(value.slice(-1));

  const result = indexSuit * raitings.length + indexRaiting;
